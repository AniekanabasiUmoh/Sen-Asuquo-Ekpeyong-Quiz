  ...
  return (
    <div className="rounded-[24px] bg-white p-6">
      ...
      <ul ref={listRef} className="mt-4 max-h-80 space-y-3 overflow-y-auto pr-1">
        {messages.length === 0 ? (
          <li className="rounded-2xl border border-dashed border-black/15 px-4 py-6 text-center text-[12px] text-[#003090]/45">
          ...
          messages.map((m) => (
            <li key={m.id} className="group flex items-start justify-between gap-2">
              <p className="min-w-0 text-[13px] leading-relaxed">
                <span className="font-semibold text-[#003090]">
                  {names[m.user_id] ?? "Supporter"}
                </span>{" "}
                <span className="text-[#1a1a2e]">{m.body}</span>
              </p>
              {isModerator || m.user_id === currentUserId ? (
                <button
                  ...
                  className="flex-none text-[11px] text-[#003090]/30 opacity-0 transition hover:text-[#c1300f] group-hover:opacity-100"
                >
                  Remove
                </button>
              ) : null}
      ...
      {currentUserId ? (
        <form onSubmit={send} className="mt-4 border-t border-black/10 pt-4">
          {error ? <p className="mb-2 text-[12px] font-semibold text-[#c1300f]">{error}</p> : null}
          <div className="flex gap-2">
            ...
            <input
              ...
              className="min-w-0 flex-1 rounded-full border border-black/15 bg-white px-4 py-2.5 text-[13px] outline-none focus:border-[#003090]"
            />
            <button
              type="submit"
              disabled={sending || !draft.trim()}
              className="flex-none rounded-full bg-[#003090] px-5 py-2.5 text-[12px] font-bold text-white transition hover:bg-[#0d2270] disabled:opacity-50"
            >
              Send
            </button>
          </div>
        ...
        <p className="mt-4 border-t border-black/10 pt-4 text-[12px] text-[#003090]/50">
          <a href="/login" className="font-semibold text-[#003090] underline-offset-4 hover:underline">
            Sign in
          </a>{" "}
