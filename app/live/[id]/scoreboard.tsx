...
export function Scoreboard({
  ...
  return (
    <div>
          ...
          return (
            <li
              ...
              className={
                ...
                (flash === r.school_id ? "ring-2 ring-gold" : "")
              }
            >
              {/* Score bar, drawn behind the content. */}
              <div
                aria-hidden="true"
                className="absolute inset-y-0 left-0 bg-primary/[0.05] transition-all duration-700"
                style={{ width: `${share}%` }}
              />
      ...
      {rows.length === 0 ? (
        <p className="rounded-[24px] bg-white px-6 py-10 text-center text-[14px] text-primary/50">
          The scoreboard appears when the match begins.
        </p>
      ) : null}

      <p className="mt-8 text-[12px] leading-relaxed text-primary/45">
