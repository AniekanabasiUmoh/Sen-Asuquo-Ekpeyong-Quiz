...
export function NewMatchForm({
  ...
  return (
    <form action={action} className="rounded-[28px] bg-white p-7">
      ...
      <p className="mt-1.5 text-[13px] leading-relaxed text-primary/55">
      ...
      <div className="mt-5 space-y-5">
        ...
        <fieldset>
          <legend className="text-[13px] font-semibold text-primary">
            Competing schools
          </legend>
              ...
              schools.map((s) => (
                <label key={s.id} className="flex items-center gap-3">
                  <input
                    ...
                    name="school_ids"
                    value={s.id}
                    className="h-4 w-4 rounded border-black/25 accent-primary"
                  />
