              ...
              <li key={b.id} className="rounded-[24px] bg-white p-6">
                ...
                {b.match_id ? (
                  <div className="mt-4 rounded-2xl bg-cream p-4">
                    <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-primary/45">
                      Browser sources for OBS or vMix
                    </p>
                    <p className="mt-2 break-all font-mono text-[12px] text-primary/70">
                      /overlay/{b.match_id}
                    </p>
                    <p className="mt-1 break-all font-mono text-[12px] text-primary/70">
                      /overlay/{b.match_id}?top=3
                    </p>
                    <p className="mt-2 text-[12px] leading-relaxed text-primary/50">
                ...
                >
                  ...
                  <button
                    ...
                    name="status"
                    value="live"
                    className="rounded-full bg-primary px-4 py-2 text-[12px] font-bold text-white transition hover:bg-navy-deep"
                  >
                    Mark live
                  </button>
                  <button
  ...
  return (
    <div className="mt-4 border-t border-black/10 pt-4">
          ...
          {links.map((l, i) => (
            ...
            >
              <span className="min-w-0 truncate text-[12px]">
                ...
                <span className="ml-2 text-primary/45">{l.url}</span>
              </span>
              <form action={removeAction}>
                ...
                <button
                  ...
                  className="flex-none text-[12px] font-semibold text-primary/45 hover:text-red-ink"
                >
                  Remove
                </button>
              </form>
      ...
      <form action={addAction} className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <select
          name="platform"
          className="rounded-full border border-black/15 bg-white px-3 py-2 text-[12px] outline-none focus:border-primary"
        >
          {PLATFORMS.map((p) => (
        ...
        <input
          ...
          name="url"
          placeholder="https://..."
          className="w-48 rounded-full border border-black/15 bg-white px-3 py-2 text-[12px] outline-none focus:border-primary"
        />
        <button
          type="submit"
