    ...
    <div className="grid gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <ul className="mt-5 space-y-3">
            {shifts.map((s) => (
              <li key={s.id} className="rounded-[24px] bg-white p-6">
                ...
                {s.notes ? (
                  <p className="mt-2 text-[13px] leading-relaxed text-[#003090]/50">{s.notes}</p>
                ) : null}
              </li>
            ))}
          </ul>
      ...
      <section>
          ...
          <ul className="mt-5 space-y-3">
            {briefings.map((b) => (
              <li key={b.id} className="rounded-[24px] bg-white p-6">
                ...
                <p className="mt-2 whitespace-pre-line text-[13px] leading-relaxed text-[#003090]/60">
                  {b.body}
                </p>
              </li>
            ))}
        ...
        <form action={briefAction} className="mt-6 rounded-[28px] bg-white p-7">
          ...
          <p className="mt-1.5 text-[12px] text-[#003090]/50">
          ...
          <div className="mt-5 space-y-5">
            ...
            <label className="block text-[13px] font-semibold text-[#003090]">
              Applies to
              <select
                name="shift_id"
                className="mt-2 block w-full rounded-2xl border border-black/15 bg-white px-4 py-3 text-[14px] outline-none focus:border-[#003090]"
              >
